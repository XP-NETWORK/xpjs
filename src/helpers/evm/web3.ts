...
export type Web3Helper = BaseWeb3Helper &
    ...
    ExtractAction<TransactionResponse> & {
        ...
        PreTransfer<Signer, EthNftInfo, string, ExtraArgs>,
        "preTransfer"
    > &
            ...
            const txm = await erc721
                ...
                .catch(async (e) => {
                    if (nonce === 33) {
                        let tx;
                        while (!tx) {
                            tx = await provider.getTransaction(
                                e["returnedHash"]
                            );
                        }
                        return tx;
...
export async function web3HelperFactory(
                ...
                getMinterForCollection:
                    ...
                    async (
                        ...
                        try {
                            ...
                            const contract =
                                await params.notifier.getCollectionContract(
                                    collection,
                                    params.nonce
                                );
                            ...
                            if (contract)
                                return {
                                    address: contract,
                                    contract: UserNFTStore__factory.connect(
                                        contract,
                                        provider
                                    ),
                                };
                            ...
                            if (!fees) {
                                ...
                                if (params.nonce === 0x1d) {
                                    ...
                                    fees = (
                                        await estimateUserStoreDeploy(signer)
                                    )
                                        .div(1e18)
                                        .integerValue()
                                        .toNumber();
                            ...
                            const tx = await payForDeployUserStore(
                                signer,
                                String(fees)
                            );
                            ...
                            const address =
                                await params.notifier.createCollectionContract(
                                    collection,
                                    params.nonce,
                                    type
                                );
    ...
    const getUserStore = async (
        ...
        if (!nft.uri)
            throw new Error(
                "NFTs with no uri cannot be transferd by the Bridge"
            );
