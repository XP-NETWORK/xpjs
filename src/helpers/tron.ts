...
// BalanceCheck<string, BigNumber>
// & WrappedBalanceCheck<string, BigNumber>
// & BatchWrappedBalanceCheck<string, BigNumber> & MintNft<string, string, null>;
...
export async function tronHelperFactory(
  ...
  return {
    ...
    nftUri: nftUri
    ,
    async transferNativeToForeign(sender: string, chain_nonce: number, to: string, value: string): Promise<[string, string]> {
      setSigner(sender)
      let res = await minter.freeze(chain_nonce, to).send({callValue: value})
      return await extractTxn(res.hash)
    },
    async unfreezeWrapped(sender: string, chain_nonce: number, to: string, value: string): Promise<[string, string]> {
      setSigner(sender)
      const res = await minter.withdraw(chain_nonce, to, value).send();
      return await extractTxn(res.hash)
    },
    async unfreezeWrappedNft(sender: string, to: string, id: BigNumber): Promise<[string, string]> {
      setSigner(sender)
      const res = await minter.withdraw_nft(to, id).send();
      return await extractTxn(res.hash)
    },
    async transferNftToForeign(sender: string, chain_nonce: number, to: string, id: EthNftInfo): Promise<[string, string]> {
      let txr;
      let ev;
      ...
      if (id.contract_type == "ERC721") {
        ...
        await erc.safeTransferFrom(sender, minter_addr, id.token, call_data).send()
      } else {
        ev = "TransferErc1155";
        const erc = await provider.contract(ERC1155_abi, id.contract)
        txr = await erc.safeTransferFrom(sender, minter_addr, id.token, EthBN.from(1), call_data).send();
      }
      ev.toString()
    ...
    async balanceWrappedBatch(address: string, chain_nonces: number[]): Promise<Map<number, BigNumber>> {
      ...
      const balance = await erc1155.balanceOfBatch(Array(chain_nonces.length).fill(address), chain_nonces).call()
