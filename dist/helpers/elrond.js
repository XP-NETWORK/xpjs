...
const ESDT_ISSUE_ADDR = new erdjs_1.Address(
...
async function elrondHelperFactory(elrondParams) {
  ...
  const doEgldSwap = async (sender, nft, value) => {
    ...
    if (
      res === undefined ||
      new bignumber_js_1.default(res.balance).lt(value)
    ) {
      const utx = new erdjs_1.Transaction({
        receiver: swapContract,
        gasLimit: new erdjs_1.GasLimit(300000000),
        value: new erdjs_1.Balance(
          erdjs_1.Egld.getToken(),
          erdjs_1.Egld.getNonce(),
  ...
  const unsignedMintNftTxn = (
    owner,
    { identifier, quantity, name, royalties, hash, attrs, uris }
  ) => {
    let baseArgs = erdjs_1.TransactionPayload.contractCall()
      .setFunction(new erdjs_1.ContractFunction("ESDTNFTCreate"))
      .addArg(
        new erdjs_1.TokenIdentifierValue(Buffer.from(identifier, "utf-8"))
      )
      .addArg(
        new erdjs_1.BigUIntValue(new bignumber_js_1.default(quantity ?? 1))
      )
      .addArg(new erdjs_1.BytesValue(Buffer.from(name, "utf-8")))
      .addArg(new erdjs_1.U64Value(new bignumber_js_1.default(royalties ?? 0)))
      .addArg(
        new erdjs_1.BytesValue(
          hash ? Buffer.from(hash, "utf-8") : Buffer.alloc(0)
        )
      )
      .addArg(
        new erdjs_1.BytesValue(
          attrs ? Buffer.from(attrs, "utf-8") : Buffer.alloc(0)
        )
      );
    for (const uri of uris) {
      baseArgs = baseArgs.addArg(
        new erdjs_1.BytesValue(Buffer.from(uri, "utf-8"))
      );
    }
  ...
  const unsignedTransferNftTxn = (
    ...
    return new erdjs_1.Transaction({
      ...
      data: erdjs_1.TransactionPayload.contractCall()
        ...
        .addArg(
          new erdjs_1.TokenIdentifierValue(
  ...
  const unsignedIssueESDTNft = (
    ...
    if (canFreeze !== undefined) {
      baseArgs = baseArgs
        ...
        .addArg(
          new erdjs_1.BytesValue(
            Buffer.from(canFreeze ? "true" : "false", "ascii")
          )
        );
    }
    if (canWipe !== undefined) {
      baseArgs = baseArgs
        ...
        .addArg(
          new erdjs_1.BytesValue(
            Buffer.from(canWipe ? "true" : "false", "ascii")
          )
        );
    }
    ...
    return new erdjs_1.Transaction({
      ...
      value: new erdjs_1.Balance(
        erdjs_1.Egld.getToken(),
        erdjs_1.Egld.getNonce(),
        new bignumber_js_1.default(ESDT_ISSUE_COST.toString())
      ),
  ...
  const unsignedSetESDTRoles = (token, target, roles) => {
    ...
    for (const role of roles) {
      baseArgs = baseArgs.addArg(
        new erdjs_1.BytesValue(Buffer.from(role, "utf-8"))
      );
    }
    return new erdjs_1.Transaction({
      receiver: ESDT_ISSUE_ADDR,
  ...
  return {
    ...
    async extractTxnStatus(txn) {
      const status = await provider.getTransactionStatus(
        new erdjs_1.TransactionHash(txn)
      );
      if (status.isPending()) {
    ...
    async transferNftToForeign(
      ...
      const txu = unsignedTransferNftTxn(
        ...
        to,
        info.native,
        new bignumber_js_1.default(txFees.toString()),
        mintWith
      );
      ...
      const txu = new erdjs_1.Transaction({
        ...
        data: erdjs_1.TransactionPayload.contractCall()
          ...
          .setArgs([
            ...
            new erdjs_1.BigUIntValue(
              new bignumber_js_1.default(nfts.length + 1)
            ),
            ...nfts.flatMap((nft) => [
              new erdjs_1.TokenIdentifierValue(esdtNftHex),
              new erdjs_1.U64Value(
                new bignumber_js_1.default(nft.native.nonce)
              ),
    ...
    async estimateValidateTransferNftBatch(_, nfts) {
      return estimateGas(
        new bignumber_js_1.default(360000000 + 5000000 * nfts.length)
      );
    },
    async estimateValidateUnfreezeNftBatch(_, nfts) {
      return estimateGas(
        new bignumber_js_1.default(340000000 + 5000000 * nfts.length)
      );
    },
    ...
    async getTokenURI(_, tokenId) {
      if (tokenId) {
        ...
        const res = await (0, axios_1.default)(url).catch(() => ({
          data: null,
        }));
