...
async function getOrCreateAssociatedTokenAccount(
  ...
  const associatedToken = await (0, spl_token_1.getAssociatedTokenAddress)(
    mint,
    owner,
    allowOwnerOffCurve
  );
  ...
  try {
    ...
    if (
      error instanceof spl_token_1.TokenAccountNotFoundError ||
  ...
  if (!account.mint.equals(mint)) throw new spl_token_1.TokenInvalidMintError();
...
async function solanaHelper(args) {
  ...
  async function getOrCreateTokenAccount(mint, owner, provider) {
    ...
    const [associatedToken] = await web3_js_1.PublicKey.findProgramAddress(
  ...
  return {
    ...
    async balance(address) {
      return new bignumber_js_1.default(
        await conn.getBalance(new web3_js_1.PublicKey(address))
      );
    },
    ...
    async transferNftToForeign(sender, chain_nonce, to, id, txFees, mintWith) {
      ...
      const bridgeContract = new anchor_1.Program(
        idl_1.IDL,
        args.bridgeContractAddr,
        provider
      );
      const [bridge, bridgeBump] = await web3_js_1.PublicKey.findProgramAddress(
        [Buffer.from("bridge")],
      ...
      const tx = await bridgeContract.methods
        .freezeNft(
          chain_nonce,
          to,
          new anchor_1.BN(txFees.toString(10)),
          mintWith,
          bridgeBump
        )
        .accounts({
